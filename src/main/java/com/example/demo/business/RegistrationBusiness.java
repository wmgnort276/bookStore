...
import com.example.demo.common.EmailValidator;
import com.example.demo.common.UserRole;
import com.example.demo.emailConfirm.EmailSender;
...
public class RegistrationBusiness {
    ...
    public String register(RegistrationRequest request) {
        boolean emailValidate = emailValidator.test(request.getEmail());
        if(!emailValidate){
            throw new IllegalStateException("Email not validate");
        }
        String token = appUserBusiness.signUpUser(
...
//        emailSender.send(request.getEmail(),buildEmail(request.getUserName(),link));
    ...
    public String confirmToken(String token){
        ...
        if(confirmationToken.getConfirmAt() != null){
            throw  new IllegalStateException("Email have already been confirm");
        }
        LocalDateTime expiredAt = confirmationToken.getExpiresTime();
        if(expiredAt.isBefore(LocalDateTime.now())){
            throw new IllegalStateException("Time expires");
        }
        confirmationBusiness.updateConfirmAt(token);
        appUserBusiness.enableUser(confirmationToken.getAppUser().getEmail());
        return "confirm";
    }
